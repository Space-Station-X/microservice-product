...
public class GlobalExceptionController {
    ...
    public ResponseEntity<ErrorResponse> handleVideoJuegoNotFoundException(VideoJuegoNotFoundException ex) {
        ErrorResponse errorResponse =
                new ErrorResponse(
                        "VIDEO_JUEGO_ERROR",
                        "Ha ocurrido un error con los video juegos",
                        Collections.singletonList(ex.getMessage())
                );
        log.warn("VideoJuego not found : {}", ex.getMessage());
        return ResponseEntity.status(HttpStatus.NOT_FOUND).body(errorResponse);
    }
